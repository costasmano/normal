...
//procedure: CreateRandomSetinConsInspection
...
If (vBDEPT#"")
	...
	If (Length:C16($s)<6)
		$s:=$s+"@"
	End if 
	QUERY SELECTION:C341([Cons Inspection:64]; [Bridge MHD NBIS:1]BDEPT:1=$s)
End if 
...
If (vBIN#"")
	...
	If (Length:C16($s)<3)
		$s:=$s+"@"
	End if 
	QUERY SELECTION:C341([Cons Inspection:64]; [Cons Inspection:64]BIN:1=$s)
End if 
