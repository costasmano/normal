...
If (False:C215)
	Mods_2005_CM12
	// Modified by: costasmanousakis-(Designer)-(5/25/2005 09:43:28) 
	Mods_2005_CM12
	// Modified by: costasmanousakis-(Designer)-(6/27/06 16:42:54)
	Mods_2006_CM05
	// Modified by: costasmanousakis-(Designer)-(3/29/2007 16:15:34)
	Mods_2007_CM07
...
If (($bCreateLetter=True:C214) & ($Recs>0))
	
	COPY NAMED SELECTION:C331([Cons Inspection:64]; "PrePrintBrgList")
	//USE SET("UserSet")
	READ ONLY:C145([Templates:86])
	QUERY:C277([Templates:86]; [Templates:86]TemplateName:2="WPtemplate_CsltInsp_NTP")
	...
	Else 
		...
		If ($Recs>3)
			//insert second page header      
			vSecondPageNo:=<>sTab+<>sTab+<>sTab+<>sTab+<>sTab+<>sTab+"- Page 2 -"+<>sCR+<>sCR
			vSecondPageSubject:="Statewide Bridge Inspection"+<>sCR+"Contract No. "+vContractNo+<>sCR+"Assignment No. "+String:C10(vAssignNo)+<>sCR+"Notice to Proceed"+<>sCR+<>sCR
		Else 
			vSecondPageNo:=""
			vSecondPageSubject:=""
		End if 
