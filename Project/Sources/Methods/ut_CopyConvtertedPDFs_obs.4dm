...
If (False:C215)
	...
	Mods_2017_03
	//  `changes 
	//use ut_CopyConvertedPDFs and not this one
End if 
Compiler_CNV
...
$OriginalsFolderPath_txt:=Select folder:C670("Select folder that has Original pictures in blobs")
...
For ($FolderLoop_L; 1; Size of array:C274($Folders_atxt))
	...
	DOCUMENT LIST:C474($SavedPicturesFolderPath_txt; $Documents_atxt)
	For ($Loop_L; 1; Size of array:C274($Documents_atxt))
		...
		Else 
			...
			READ PICTURE FILE:C678($SavedPicturesFolderPath_txt+$Documents_atxt{$Loop_L}; $Picture_pct)
			$Line_txt:=$Line_txt+String:C10(Get document size:C479($SavedPicturesFolderPath_txt+$Documents_atxt{$Loop_L}))+Char:C90(Tab:K15:37)
			...
			Repeat 
				...
				ON ERR CALL:C155("CNV_CopyError")
				COPY DOCUMENT:C541($SavedPicturesFolderPath_txt+$Documents_atxt{$Loop_L}; $CopyToPath_txt+String:C10($PhotoID_L)+"."+$Parts_atxt{2})
				
				ON ERR CALL:C155("")
			Until (TryAgain_L=2) | (TryAgain_L=3) | (TryAgain_L=4)
			If (TryAgain_L=2)
				SEND PACKET:C103($doc_tm; String:C10($InspectionID_L)+Char:C90(Tab:K15:37)+$Documents_atxt{$Loop_L}+Char:C90(Tab:K15:37)+$SavedPicturesFolderPath_txt+Char:C90(Tab:K15:37)+$CopyToPath_txt+Char:C90(Carriage return:K15:38))
			Else 
				//SEND PACKET($Errordoc_tm;Char(Carriage return)+String($InspectionID_L)+Char(Tab)+$Documents_atxt{$Loop_L}+Char(Tab)+$SavedPicturesFolderPath_txt+Char(Tab)+$CopyToPath_txt++"Unknown error code listed above"+Char(Carriage return))
			End if 
			...
			VARIABLE TO BLOB:C532($Picture_pct; $Blob_blb; *)
			
			BLOB TO DOCUMENT:C526($NewDocPath_txt; $Blob_blb)
