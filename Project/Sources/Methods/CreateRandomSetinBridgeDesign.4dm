...
If (False:C215)
	...
	// Modified by: Costas Manousakis-(Designer)-(12/30/13 16:46:23)
	Mods_2013_12
...
If ((vAddTo=True:C214) & (Records in set:C195("Random Set")=0))
	CREATE SET:C116([Bridge Design:75]; "Random Set")
End if 
...
If (vBDEPT#"")
	...
	If (Length:C16($s)<6)
		$s:=$s+"@"
	End if 
	QUERY SELECTION:C341([Bridge Design:75]; [Bridge MHD NBIS:1]BDEPT:1=$s)
End if 
...
If (vBIN#"")
	...
	If (Length:C16($s)<3)
		$s:=$s+"@"
	End if 
	QUERY SELECTION:C341([Bridge Design:75]; [Bridge Design:75]BIN:1=$s)
End if 
...
CREATE SET:C116([Bridge Design:75]; "Found Bridge Recs Set")
If (Records in set:C195("Random Set")=0)
	If (vAddTo=False:C215)
		CREATE EMPTY SET:C140([Bridge Design:75]; "Random Set")
	End if 
End if 
