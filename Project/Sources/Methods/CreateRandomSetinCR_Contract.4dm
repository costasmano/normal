...
If (vContractNo#"")
	...
	If (Length:C16($s)<3)
		$s:=$s+"@"
	End if 
	QUERY SELECTION:C341([Conslt Rating:63]; [Conslt Rating:63]ContractNoRat:14=$s)
End if 

If (vsAssignNo#"")
	QUERY SELECTION:C341([Conslt Rating:63]; [Conslt Rating:63]AssignNoRat:12=Num:C11(vsAssignNo))
End if 

ORDER BY:C49([Conslt Rating:63]; [Conslt Rating:63]AssignNoRat:12)
