...
If (False:C215)
	...
	$text:=$text+"Each record should consist of a BIN, Item 8, and an AASHTO rating."
	ALERT:C41($text)
	...
	ARRAY REAL:C219($aAASHTO; 0)
	ARRAY BOOLEAN:C223($aXSelect; 0)
	ARRAY TEXT:C222($aBIN; 0)  //Command Replaced was o_ARRAY string length was 3
	SELECTION TO ARRAY:C260([Bridge MHD NBIS:1]Item8:206; $aItem8; [Bridge MHD NBIS:1]AASHTO:5; $aAASHTO; [Bridge MHD NBIS:1]FHWA Select:4; $aXSelect)
	...
	While (OK=1)
		...
		End if 
		If ($PrintWarn)
			$text:=String:C10($i)+<>sTab+$BIN+<>sTab+$Item8+$text+<>sCR
			SEND PACKET:C103($dUpdateRpt; $text)
		End if 
	...
	If ($j>0)
		...
		SEND PACKET:C103($dUpdateRpt; $text)
		While ($j>0)
			$text:=<>sTab+$aBIN{$j}+<>sTab+$aItem8{$j}+<>sCR
			SEND PACKET:C103($dUpdateRpt; $text)
