...
//Method: MAP_BuildAncGeoJSON
//Description
// build a GeoJson file from the current list of anc structures
// uses parameter MAP_AncStructureFlds
// Parameters
// ----------------------------------------------------
If (False:C215)
	// ----------------------------------------------------
	...
	//User (4D) : Designer
	//Created : 
	//Date and time: Nov 3, 2023, 10:48:38
	Mods_2023_11
	// ----------------------------------------------------
...
$currsel_o:=Create entity selection:C1512([LSS_Inventory:165])
$zeroloc_o:=$currsel_o.query(Field name:C257(->[LSS_Inventory:165]LSS_Latitude_r:19)+" = 0 or "+\
Field name:C257(->[LSS_Inventory:165]LSS_Longitude_r:18)+" = 0")
...
If ($continue_b)
	...
	$mapFieldsinfo_txt:=ut_GetSysParameter("MAP_AncStructureFlds")
	...
	Else 
		...
		For each ($prop_o; $properties_c)
			...
			$coords_c:=New collection:C1472($prop_o.LSS_Longitude_r; $prop_o.LSS_Latitude_r)
