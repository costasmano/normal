...
// Date and time: 06/01/06, 14:49:22
// ----------------------------------------------------
// Description
// 
//
...
If (Size of array:C274($ParsedText_atxt)=1)
	If (Position:C15("N.H."; $ParsedText_atxt{1})>0)
		[PRJ_ReportData:118]RPT_Source_s:7:=$ParsedText_atxt{1}
	Else 
				...
				Case of 
					...
					: (Position:C15("Approved"; $ParsedText_atxt{1})>0)
						...
						If (String:C10($Test_d; 4)=$StringDate_s) | (String:C10($Test_d)=$StringDate_s) | (String:C10($Test_d)=$StringDate_s)  //we have an approval
							...
							DELETE FROM ARRAY:C228($ParsedText_atxt; 1; 1)
						End if 
					: (Position:C15("N.H."; $ParsedText_atxt{1})>0)
						[PRJ_ReportData:118]RPT_Source_s:7:=$ParsedText_atxt{1}
						$StringDate_s:=$ParsedText_atxt{2}
						$Test_d:=Date:C102($StringDate_s)
						DELETE FROM ARRAY:C228($ParsedText_atxt; 1; 1)
					...
					: (Position:C15("BOR"; $ParsedText_atxt{1})>0)
						SAVE RECORD:C53([PRJ_ReportData:118])
						UNLOAD RECORD:C212([PRJ_ReportData:118])
						C_LONGINT:C283($Position_l)
						$Position_l:=Position:C15("BOR"; $GeotechReport_txt)
						
						$GeotechReport_txt:=Substring:C12($GeotechReport_txt; $Position_l)
						ut_PRJImportCreateBorePlan($GeotechReport_txt; "Boring Plan")
