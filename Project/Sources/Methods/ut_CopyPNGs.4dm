...
If (SQLConnectionMade_b)
	...
	APPEND TO ARRAY:C911($ExtensionsToCopy_atxt; "pdf")
	
	//clone_sqlConnect ("designer";"caesar";"146.243.135.242:19812")
	Repeat 
		...
		C_LONGINT:C283($Loop_L; $InspectionID_L; $PhotoID_L)
		$ConvertedFolderPath_txt:=Select folder:C670("Select folder to that has Move To Pictures [MoveTo]")
		
		If (OK=1)
			$SavedPDFSFolderPath_txt:=Select folder:C670("Select folder that has previously saved PNGs [Originals:PNGs to Move:PDFConverted YYY-MM-DD ")
			If (OK=1)
			...
			End if 
		End if 
		CONFIRM:C162("Process another folder after "+$SavedPDFSFolderPath_txt; "Yes"; "No")
		
	Until (OK=0)
	SQL LOGOUT:C872
	CLOSE DOCUMENT:C267(CNV_Doc_tm)
	ALERT:C41("completing processing "+$SavedPDFSFolderPath_txt)
End if 
//End ut_CopyPNGs