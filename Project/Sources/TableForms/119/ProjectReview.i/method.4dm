
If (False:C215)
	// ----------------------------------------------------
	// Form Method: ProjectReview.i
	// User name (OS): cjmiller
	// Date and time: 01/26/06, 10:32:08
	// ----------------------------------------------------
	// Description
	// 
	//
	// Parameters
	// ----------------------------------------------------
	
	Mods_2007_CJMv2  //r060   `05/16/07, 15:13:40 `Add code to control access 
	// Modified by: costasmanousakis-(Designer)-(6/8/2007 16:43:31)
	Mods_2007_CM11
	// Modified by: costasmanousakis-(Designer)-(8/23/2007 14:22:24)
	Mods_2007_CM12_5301
	// Modified by: costasmanousakis-(Designer)-(10/29/2007 08:56:36)
	Mods_2007_CM12_5302
	Mods_2007_CJM_v54  //r001 `11/13/07, 17:08:17`Make first selected record allow for project approval entry
	// Modified by: costasmanousakis-(Designer)-(10/8/08 10:25:59)
	Mods_2008_CM_5404  // ("ACCESS")
	// Modified by: costasmanousakis-(Designer)-(5/6/09 10:51:24)
	Mods_2009_05
	//Replaced the Title of the PRJ_Approval field to " " to avoid having phantom highlite appear on Win machines
	//when the box was checked.
	Mods_2011_06  // CJ Miller`06/27/11, 16:26:19      ` If [PRJ_ProjectDetails] locked set to non-enterable
End if 
C_BOOLEAN:C305(PRJ_2ndAddSubRecord_b)
Case of 
	: (Form event code:C388=On Load:K2:1)
		utl_SetSpellandContextMenu
		ut_SetStringOrTextFilter(->[PRJ_StructuralReview:119]SR_Comments_txt:5)
		
		C_LONGINT:C283($Color_l)
		$Color_l:=<>PRJ_Color_editable
		
		OBJECT SET ENABLED:C1123(bValidate; True:C214)  // Command Replaced was o_ENABLE BUTTON 
		OBJECT SET ENABLED:C1123(Button1; True:C214)  // Command Replaced was o_ENABLE BUTTON 
		OBJECT SET ENABLED:C1123(Button9; True:C214)  // Command Replaced was o_ENABLE BUTTON 
		OBJECT SET ENABLED:C1123(btnShowAddress; True:C214)  // Command Replaced was o_ENABLE BUTTON 
		
		OBJECT SET VISIBLE:C603(*; "ReviewerDD@"; True:C214)
		OBJECT SET ENTERABLE:C238(*; "SR@"; True:C214)
		If (Record number:C243(Current form table:C627->)=New record:K29:1)
			If (PRJ_ReturnAccessAllowed)
				
				If (PRJ_2ndAddSubRecord_b)
					PRJ_2ndAddSubRecord_b:=False:C215
					CANCEL:C270
				Else 
					[PRJ_StructuralReview:119]SR_StructureReviewOrder_l:3:=PRJ_LastReview_l+1
					[PRJ_StructuralReview:119]PRJ_ProjectID_l:1:=[PRJ_ProjectDetails:115]PRJ_ProjectID_l:1
					[PRJ_StructuralReview:119]RV_ReviewID_l:2:=PRJ_LastReviewer_l
					[PRJ_StructuralReview:119]SR_Received_d:7:=Current date:C33(*)
				End if 
			Else 
				CANCEL:C270
			End if 
		Else 
			If (PRJ_ReturnAccessAllowed)
				If (Selected record number:C246([PRJ_StructuralReview:119])=1)
					OBJECT SET VISIBLE:C603(*; "PRJ_Approval@"; True:C214)
				Else 
					OBJECT SET VISIBLE:C603(*; "PRJ_Approval@"; False:C215)
				End if 
			Else 
				OBJECT SET VISIBLE:C603(*; "PRJ_Approval@"; False:C215)
				OBJECT SET ENABLED:C1123(bValidate; False:C215)  // Command Replaced was o_DISABLE BUTTON 
				OBJECT SET ENABLED:C1123(bDelete; False:C215)  // Command Replaced was o_DISABLE BUTTON 
				OBJECT SET ENABLED:C1123(PRJ_StructReviewers_atxt; False:C215)  // Command Replaced was o_DISABLE BUTTON 
				OBJECT SET ENABLED:C1123(PRJ_GradingStatus_atxt; False:C215)  // Command Replaced was o_DISABLE BUTTON 
				OBJECT SET ENTERABLE:C238(*; "SR@"; False:C215)
				OBJECT SET ENABLED:C1123(Button1; False:C215)  // Command Replaced was o_DISABLE BUTTON 
				OBJECT SET ENABLED:C1123(Button9; False:C215)  // Command Replaced was o_DISABLE BUTTON 
				$Color_l:=<>PRJ_Color_Not_editable
				OBJECT SET ENABLED:C1123(*; "ReviewerDD@"; False:C215)  // Command Replaced was o_DISABLE BUTTON 
				OBJECT SET ENABLED:C1123(btnShowAddress; False:C215)  // Command Replaced was o_DISABLE BUTTON 
			End if 
			
		End if 
		PRJ_FillReviewersDisplay(->[PRJ_StructuralReview:119]; [PRJ_StructuralReview:119]RV_ReviewID_l:2)
		OBJECT SET RGB COLORS:C628(*; "SR@"; Col_paletteToRGB(Abs:C99($Color_l)%256); Col_paletteToRGB(Abs:C99($Color_l)\256))  // **Replaced o OBJECT SET COLOR(*; "SR@"; $Color_l)
		
	: (Form event code:C388=On Validate:K2:3)
		PRJ_LastReviewer_l:=[PRJ_StructuralReview:119]RV_ReviewID_l:2
		PRJ_SetProjectChangesMade(->[PRJ_StructuralReview:119]; ->[PRJ_StructuralReview:119]SR_ModifiedBy_s:8; ->[PRJ_StructuralReview:119]SR_ModifiedTimeStamp_s:9)
		
End case 
//End Form Method: ProjectReview.i