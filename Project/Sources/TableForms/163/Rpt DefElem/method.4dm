...
Else 
	Case of 
		...
		: (DEFELMTPRINT_Area_txt="Detail")
			C_LONGINT:C283($Loc_L; $DPos_L; $HPos_L)
			$Loc_L:=Find in array:C230(v_162_001_aL; [LSS_ElementInspection:163]LSS_ElementId_L:2)
			vElmDescr:=""
			If ($Loc_L>0)
				If (LSS_Headers_atxt{$Loc_L}=v_162_002_atxt{$Loc_L})
					vElmDescr:=LSS_Headers_atxt{$Loc_L}
				Else 
					$HPos_L:=Position:C15(" "; LSS_Headers_atxt{$Loc_L})
					$DPos_L:=Position:C15(" "; v_162_002_atxt{$Loc_L})
					If (Position:C15("blank element"; v_162_002_atxt{$Loc_L})>0)
						vElmDescr:=Substring:C12(v_162_002_atxt{$Loc_L}; 1; $DPos_L)+Substring:C12(LSS_Headers_atxt{$Loc_L}; $HPos_L+1)+"/"+[LSS_ElementInspection:163]LSS_Description_txt:7
					Else 
						vElmDescr:=Substring:C12(v_162_002_atxt{$Loc_L}; 1; $DPos_L)+Substring:C12(LSS_Headers_atxt{$Loc_L}; $HPos_L+1)+"/"
						vElmDescr:=vElmDescr+Substring:C12(v_162_002_atxt{$Loc_L}; $DPos_L+1)
					End if 
				End if 
			End if 
			vElmDescr:=vElmDescr+" "+vsContText
			//ElmCmts_txt:=[LSS_ElementInspection]LSS_Remarks_txt
			
			
		: (DEFELMTPRINT_Area_txt="Footer")
		: (DEFELMTPRINT_Area_txt="Break0")
			//I do not think we want to print comments. If we do just un-comment below
			...
			If ($hght1>$MaxCmtHt)
				...
				End while 
				ElmCmts_txt:=Substring:C12($orig_txt; 1; $CurPos_L)
				$orig_txt:=Substring:C12($orig_txt; $CurPos_L)
				OBJECT GET BEST SIZE:C717(*; "ElmCmts_txt"; $wdth1; $hght1; $fixed_wdth1)
